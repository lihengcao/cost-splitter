...
function calculateGrandTotals(subtotal) {
    ...
    const total = addBefore ? (subtotal + adder) * multiplier : (subtotal * multiplier) + adder;
    document.getElementById("totalDisplay").innerText = total.toFixed(2);
    
    return { adder, multiplier, addBefore };
}

function splitCostsProportionally(people, subtotal, globals, costTable, paymentTable) {
    ...
    itemRows.forEach(row => {
        ...
        let checkedIndices = [];
        checkboxes.forEach((cb, idx) => { if (cb.checked) checkedIndices.push(idx); });

        const divisor = checkedIndices.length || people.names.length || 1;
        const split = itemCost / divisor;

        if (checkedIndices.length === 0) {
            for (let i = 0; i < personBaseCosts.length; i++) personBaseCosts[i] += split;
        } else {
    ...
    for (let i = 0; i < people.names.length; i++) {
        const shareRatio = subtotal === 0 ? (1 / (people.names.length || 1)) : (personBaseCosts[i] / subtotal);
        let finalVal = 0;
        
        if (globals.addBefore) {
            finalVal = globals.multiplier * (personBaseCosts[i] + (shareRatio * globals.adder));
        } else {
            finalVal = (personBaseCosts[i] * globals.multiplier) + (shareRatio * globals.adder);
        }
...
function addNewRow() {
    ...
    const n = row.insertCell(1); n.className = "itemName"; n.contentEditable = true; n.innerText = "Item " + id;
    const c = row.insertCell(2); c.className = "cost"; c.contentEditable = true; c.innerText = "0";
    row.insertCell(3).className = "spacer";
    
    // Add person checkboxes
    const peopleCount = document.getElementById("costTable").rows[0].querySelectorAll('.person-header').length;
...
function reconstructUIFromState(state) {
    ...
    while(costHead.lastElementChild.classList.contains('person-header')) { costHead.deleteCell(-1); }
    while(payHead.cells.length > 0) { payHead.deleteCell(0); }
    ...
    items.forEach(item => {
        ...
        const n = row.insertCell(1); n.className = "itemName"; n.contentEditable = true; n.innerText = name;
        const c = row.insertCell(2); c.className = "cost"; c.contentEditable = true; c.innerText = cost;
        row.insertCell(3).className = "spacer";
...
function bindInputEvents(root = document) {
    const targets = root === document 
        ? document.querySelectorAll(".itemName, .cost, .person-header, #globalAdder, #globalMultiplier")
...
window.addEventListener('load', () => {
    ...
    if (b64State) {
        ...
        try { reconstructUIFromState(JSON.parse(legacyState)); } catch(e) {}
    }
